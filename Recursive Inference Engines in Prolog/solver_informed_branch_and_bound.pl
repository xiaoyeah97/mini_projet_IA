...
__HISTORY__
   ...
   * June 2019:  cloning the tracing version then removing the traces
...
:- module(solver_informed_branch_and_bound, [ solver/4
                                            , solver/5
                                            , solver/6
...
% It simply calls its explicit counterpart, 'solver/7', by initialising inner parameters, namely the (empty) list of ancestors for directed-cycle detection.
%
...
%
solver(F, _, _, S, _, [], _) :-     % There is no transition in the solution...
...
%
select_best(F, O, H, AS,
            transition(D, NS, C, _), 
            no_solution(UB), 
            solution(C_SS, SS)) :-
   plus(UB_NS, C, UB),                       % (Guaranteed to be positive.)  
   solver(F, O, H, NS, UB_NS, SS_NS, AS),    % Search for a solution starting at this successor.
...
select_best(F, O, H, AS,
   ...
   solver(F, O, H, NS, UB_NS, SS_NS, AS),
