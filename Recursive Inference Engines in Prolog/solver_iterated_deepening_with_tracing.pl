...
:- module(solver_iterated_deepening_with_tracing, [ solver/4
                                                  , solver/5
                                                  ]).
:- meta_predicate solver(1, 4, ?, ?).
:- meta_predicate solver(1, 4, ?, ?, ?).

:- use_module(solver_depth_limit_with_tracing, [ solver/5 as solver_depth_limited ]).
...
meta_solver(_, _, _, _, D_c, D_max) :-
   ...
   write("\nMETA SEARCH MAXIMAL DEPTH REACHED...\n"),
